...
class BqplotDotPlotLayerArtist(BqplotHistogramLayerArtist):
    ...
    def _update_size(self, arg=None):
        ...
        scaling = 0.7
        heights = []
        if self._viewer_state.x_max is not None and self._viewer_state.x_min is not None:
            x_range = max(self._viewer_state.x_max - self._viewer_state.x_min, 1)
            print("X range:")
            print(x_range)
            if x_range == 0:
                self._viewer_state.x_max += 1
                x_range = 1

            x_pixel_height = (self._viewer_state.viewer_width + self.view.figure.fig_margin["left"]) / x_range
